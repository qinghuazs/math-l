...
(function (CW) {
  ...
  var scene = {
    ...
    steps: [
      {
        narration: '考试丢分最多的不是不会，而是易错点没防住。第一组易错：<b>漏乘</b>——方程两边同乘时，常数项也要乘！2x+y=3 两边乘 3，应得 6x+3y=9，右边的 3 常被漏乘；<b>减式忘变号</b>——两式相减时，减数的每一项都要变号：(3x+2y)−(3x−2y)=4y，不是 0！',
        enter: function () {
      ...
      {
        narration: '第二组易错：<b>解的书写</b>——方程组的解是一对值，必须用大括号联立写出 x=2、y=1，只写一个数不完整；<b>检验只代一个方程</b>——方程组的解要同时满足两个方程，检验也必须两个都代！',
        enter: function () {
          S.remove('s4-title'); S.remove('s4-e1-w'); S.remove('s4-e1-r');
          S.remove('s4-e2-w'); S.remove('s4-e2-r');
      ...
      {
        narration: '第三组易错：<b>应用题不检验合理性</b>——解出负的人数、超过总量的时间，说明列错了方程却照样作答，这是最可惜的丢分。把这五个易错点记牢，本章就稳了。',
        enter: function () {
          S.remove('s4-title2'); S.remove('s4-e3-w'); S.remove('s4-e3-r');
          S.remove('s4-e4-w'); S.remove('s4-e4-r');
