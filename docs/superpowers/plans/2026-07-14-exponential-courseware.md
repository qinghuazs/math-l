...
2. **测试**：纯逻辑（tween、director、场景结构）用 Node 内置测试跑：`node --test tests/*.test.js`。DOM/画板视觉用浏览器走查（chrome-devtools MCP 或手动打开），验收标准=控制台零报错+截图符合预期。要求 Node ≥ 18（仅开发机需要）。
...
```bash
node --test tests/*.test.js   # 最后回归：全部 pass
...
- [ ] `node --test tests/*.test.js` 全部通过（8 个测试）；
